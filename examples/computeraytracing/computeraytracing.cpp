...
public:
	...
	void buildCommandBuffers()
	{
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
		{
			...
			vkCmdBeginRenderPass(drawCmdBuffers[i], ??, VK_SUBPASS_CONTENTS_INLINE);
			...
			vkCmdBindDescriptorSets(drawCmdBuffers[i], ??, ??, 0, 1, ??, 0, NULL);
			vkCmdBindPipeline(drawCmdBuffers[i], ??, ??);
			vkCmdDraw(drawCmdBuffers[i], ??, ??, 0, 0);
	...
	void buildComputeCommandBuffer()
	{
		...
		// TODO 5.1
		vkCmdBindPipeline(compute.commandBuffer, ??, ??);
		vkCmdBindDescriptorSets(compute.commandBuffer, ??, ??, 0, 1, ??, 0, 0);

		vkCmdDispatch(compute.commandBuffer, ??, ??, 1);
	...
	void prepareStorageBuffers()
	{
		...
		vulkanDevice->createBuffer(??, ??, &stagingBuffer, storageBufferSize, sceneObjects.data());
		// TODO 2.5.2: create a device local buffer
		vulkanDevice->createBuffer(??, ??, &compute.objectStorageBuffer, storageBufferSize);
		VkCommandBuffer copyCmd = vulkanDevice->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		VkBufferCopy copyRegion = { 0, 0, storageBufferSize};
		// TODO 2.5.3: copy data from staging buffer to device buffer
		vkCmdCopyBuffer(copyCmd, ??, ??, 1, &copyRegion);
	...
	void prepareGraphics()
	{
		...
		VkPipelineRasterizationStateCreateInfo rasterizationState = vks::initializers::pipelineRasterizationStateCreateInfo(??, VK_CULL_MODE_FRONT_BIT, VK_FRONT_FACE_COUNTER_CLOCKWISE, 0);
	...
	void prepareCompute()
	{
		...
		std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
			...
			vks::initializers::descriptorSetLayoutBinding(??, ??, 1),
			vks::initializers::descriptorSetLayoutBinding(??, ??, 2),
		};
		VkDescriptorSetLayoutCreateInfo descriptorLayout = vks::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
		...
		std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
			...
			vks::initializers::writeDescriptorSet(compute.descriptorSet, ??, ??, &compute.uniformBuffer.descriptor),
			vks::initializers::writeDescriptorSet(compute.descriptorSet, ??, ??, &compute.objectStorageBuffer.descriptor),
		};
	...
	void draw()
	{
		...
		vkWaitForFences(device, ??, ??, VK_TRUE, UINT64_MAX);
		...
		std::vector<VkPipelineStageFlags> graphicsSubmitPipelineFlags = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, ?? };
